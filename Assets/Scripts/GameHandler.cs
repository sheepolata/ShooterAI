...
public class GameHandler : MonoBehaviour
{
    ...
    void Update() {
        ...
        Vector2 campos = camTarget.getPosition();
        
        // Offset camera according to look direction
        if(offsetCamOnLookDir){
            Vector2 offset = new Vector2(Mathf.Cos((camTarget.getLookDir()+90f) * Mathf.Deg2Rad), Mathf.Sin((camTarget.getLookDir()+90f) * Mathf.Deg2Rad));
